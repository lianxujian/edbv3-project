  ...
  export default {
    ...
    methods: {
      ...
      amendAddName() {
        ...
        self.$axios({
          method: 'post',
          url: process.env.API_HOST + 'com.edb01.upms.service.DepartmentService/update',
          data: {clToken: self.token, departmentId: self.departmentId, departmentName: self.addDep.departmentName},
      ...
      amendNode() {
        ...
        self.$axios({
          method: 'post',
          url: process.env.API_HOST + 'com.edb01.upms.service.DepartmentService/get',
          data: {departmentId: self.departmentId, clToken: self.token},
          transformRequest: [function (data) {
            let newData = ''
            for (var k in data) {
              newData += encodeURIComponent(k) + '=' + encodeURIComponent(data[k]) + '&'
      ...
      delNode() {
        ...
        self.$confirm('是否删除部门?', '提示', {
          ...
          self.$axios({
            method: 'post',
            url: process.env.API_HOST + 'com.edb01.upms.service.DepartmentService/delete',
            data: {departmentId: self.departmentId, clToken: self.token},
            transformRequest: [function (data) {
              let newData = ''
              for (var k in data) {
      ...
      addName() {
        ...
        self.$axios({
          method: 'post',
          url: process.env.API_HOST + 'com.edb01.upms.service.DepartmentService/add',
          data: self.addDep,
          transformRequest: [function (data) {
            let newData = ''
            for (var k in data) {
    ...
    mounted() {
      ...
      self.queryCompany = (parameter) => {
        self.company.clToken = self.token
        self.$axios({
          method: 'post',
      ...
      self.queryData = (pars) => {
        self.$axios({
          ...
          data: {companyId: pars, clToken: self.token},
          transformRequest: [function (data) {
            let newData = ''
            for (var k in data) {
              newData += encodeURIComponent(k) + '=' + encodeURIComponent(data[k]) + '&'
          ...
          .then(function (res) {
            if (res.data.code == 0) {
              self.data = res.data.result.nodes
            } else {
              self.$message({
                message: res.data.msg,
                type: 'warning'
      ...
      self.$axios({
        method: 'post',
        url: process.env.API_HOST + 'com.edb01.upms.service.LoginRegistService/isAdmin',
