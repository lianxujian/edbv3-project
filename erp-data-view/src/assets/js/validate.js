...
export function turnTimestamp3(time) {
  ...
  return turnSingle(timeArr1[0]*1)+turnSingle(timeArr1[1]*1)+turnSingle(timeArr1[2]*1)+turnSingle(timeArr2[0]*1)+turnSingle(timeArr2[1]*1)+turnSingle(timeArr2[2]*1);
}
...
export function returnOneFloat(value){
  ...
  // alert(value.toString().indexOf("3"));
  // alert(value.toString().substring(0,value.toString().indexOf(".")+2));
...
export function keepOneBits(x) {
  ...
  var f = parseFloat(x);
  //var f = Math.floor(x*100)/100;
  var f = Number(x.toString().match(/^\d+(?:\.\d{0,1})?/)) ;
  var s = f.toString();
  var rs = s.indexOf('.');
  if (rs < 0) {
    rs = s.length;
