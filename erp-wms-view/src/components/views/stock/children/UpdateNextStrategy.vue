        ...
        <el-col :span="18">
          <div class="wrapper-one">
            <div class="fix">
              ...
              <!--<el-select v-model="updateData.soldoutType"  filterable placeholder="全部" :disabled="isShow" style="width:50%;width:200px;">
                <el-option
                  v-for="item in types"
                  :key="item.value"
                  :label="item.label"
        ...
        <el-col :span="18">
          <div class="wrapper-one">
            <div class="fix" style="margin-bottom: 10px">
              ...
              <!--<span v-if="isShow==true"><el-button  round>添加</el-button></span>
              <span v-else @click="addClickExpress()" ><el-button type="primary" round>添加</el-button></span>&lt;!&ndash;v-if="isShow==true"&ndash;&gt;-->
            ...
            <template slot-scope="scope">
              <div v-if="scope.row.isShowKit1">
               <!-- <el-badge  class="item" style="margin:10px 10px 5px 0;cursor: pointer" v-for="(item,index) in  scope.row.soldoutStrategySectionBeanList" :key="index"  >
                ...
                <el-badge value="X" class="item" style="margin:10px 20px 15px 0;cursor: pointer" v-for="(item,index) in scope.row.sectionVOS" :key="index" @click.native="clickDelOne(index,scope.row)">
                  <el-button style="width:auto!important;" size="small">{{item.sectionName}}</el-button>
                </el-badge>
              </div>
                <div v-if="!scope.row.isShowKit1">
                  <el-badge  class="item" style="margin:10px 10px 5px 0;cursor: pointer" v-for="(item,index) in  scope.row.sectionVOS" :key="index">
                    <el-button style="width:auto!important;" size="small">{{item.sectionName}}</el-button>
                  </el-badge>
                </div>
              <div v-if="scope.row.isShowKit1">
                <p class="cen-g">
                  <i class="el-icon-circle-plus" @click="setOrder(scope.row,scope.$index);shelves=true"></i>
                </p>
              </div>
            </template>
          </el-table-column>
  ...
  export default {
    data(){
      return {
        ...
        updateData:{
          ...
          excellentPri:'',//拣货路径最优
          emptyPri:'',//清空库位
          SoldoutStrategyStorageBeanList:[],//仓库
        },//修改
        isShowButton:false,//禁用按钮
        isChangedata:[],//校验
        old:[],//出库单类型
      }
    } ,
    mounted(){
      this.getName(); //根据商户id查询启用仓库列表。
      ...
      var self = this;
      var paramData = {
        soldoutStrategyId:self.$router.history.current.params.soldoutStrategyId,//波次id
      }
      self._post('com.edb01.erp.wms.service.api.WmSoldoutStrategyService/' + this.version +'/findSoldoutStrategy',paramData)
        .then(function (response) {
          if (response.data.code == 0) {
            self.updateData=response.data.result;
            self.priArray = response.data.result.priArrayStr;
            self.tableData=response.data.result.storageVOS;
            self.soldoutStrategyId=response.data.result.soldoutStrategyId;
            ...
            for(var item of self.tableData) {
              self.sectionVOS=item.sectionVOS
            }
            var tableLeft = [
              {id:1,name: '近有效期先出'},
              {id:2,name: '先进先出'},
            ...
            self.tableLeft = tableLeft;
              self.sectionVOS.forEach(item => {
                let obj = {storageSectionId:item.storageSectionId};
                 self.old.push(obj);
              });
           self.storageId=item.storageId
            self.updateData.soldoutTypeList.forEach((item)=>{
              $('#example-post').multiselect('select',item);
            })
    ...
    methods:{
      ...
      addClickExpress: function () {
        if(this.updateData.strategyName == '系统默认下架策略'){ // 判断当前是否是系统默认
          this.isShow = true;
          this.isShowButton=true;
        }else{
          if(this.tableData.length<1){
            this.tableData.push({storageId: '',isShowKit1:true,sectionVOS: []})
      ...
      sureData() {
        this.strateg1.forEach(item => {
          this.tableData[this.indexTag].sectionVOS.push(item);
        })
        this.shelves = false;
        this.$nextTick(() => {
      ...
      clickDel(tableIndex,index) {
        this.tableData[tableIndex].sectionVOS.splice(index,1)
      },
      ...
      addAll(){
        ...
        arr2.push(arr1.indexOf('近有效期先出')+1);
        arr2.push(arr1.indexOf('先进先出')+1);
        arr2.push(arr1.indexOf('拣货路径最优')+1);
        arr2.push(arr1.indexOf('清空库位')+1);
        var self=this;
        let data= [];
        self.tableData.forEach((item)=>{
          ...
          item.soldoutStrategySectionBeanList=item.sectionVOS
          delete item.sectionVOS
        })
        ...
        if ($('#example-post').val().length == self.types.length) {
          ...
          self.updateData.soldoutTypes= value
        }else {
          self.updateData.soldoutTypes = $('#example-post').val();
        }

        var updateDataW = {
          soldoutStrategyId:self.soldoutStrategyId,//下架策略id
          strategyName:self.updateData.strategyName,//策略名
          soldoutTypes:self.updateData.soldoutTypes,//出库单类型
          priArrayStr:arr2.join(','),
          SoldoutStrategyStorageBeanList:JSON.stringify(self.tableData),
        }
        if(this.isShow == true){ //系统默认置为空出库单类型
          updateDataW.soldoutTypes = '';
        }
        if(self.tableData.length==0){
          updateDataW.SoldoutStrategyStorageBeanList=''
        }
        self._post('com.edb01.erp.wms.service.api.WmSoldoutStrategyService/'+this.version+'/updateSoldoutStrategy',updateDataW)
          .then(function (res) {
            if (res.data.code == 0) {
              self.$message({
                type: 'success',
            ...
            }else if(res.data.code==1100000009){
              self.$message({
                message: '表Id为空！',
                type: 'warning'
              });
