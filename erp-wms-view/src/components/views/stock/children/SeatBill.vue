  ...
  <div id="seatBill">
    ...
    <div class="pTop">
      <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item><a @click="showParentBlock">库位库存查询</a></el-breadcrumb-item>
    ...
    <div style="height: 40px;line-height: 40px;padding-left: 10px;">
      ...
      <span style="margin-left: 85px">商品名称：{{ goodsName }}</span>
      <span style="margin-left: 85px">批次：{{ batchNo }}</span>
      <span style="margin-left: 85px">仓库：{{ storageName }}</span>
      <span style="margin-left: 85px">库区：{{ sectionName }}</span>
      <span style="margin-left: 85px">库位：{{ storageBinName }}</span>
      <span style="margin-left: 85px" v-if="binType == 1">库位类型：正品</span>
      <span style="margin-left: 85px" v-if="binType == 2">库位类型：次品</span>
    </div>
    ...
    <div class="pDown">
      <el-table id="tableH" :data="tableData" border :max-height="table_h" style="width: 100%">
        ...
        <el-table-column :label="camponName" align="right">
          <template slot-scope="scope">
            {{ scope.row.camponQty }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" v-if="isShow" align="center">
          <template slot-scope="scope">
            <el-button
              ...
              type="text"
              size="small">
              <a href="javascript" v-if="scope.row.orderType == 1 && authorityCode.WMS_SEAT_CAMPON">解除预占</a>
            </el-button>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    data(){
      return{
        ...
        batchNo: this.params.id.batchNo,//批次
        storageName: this.params.id.storageName,//仓库名称
        sectionName: this.params.id.sectionName,//库区名称
        storageBinName: this.params.id.storageBinName,//库位名称
        binType: this.params.id.binType,//库位类型

        camponName: '预占数量',//动态加载列头名称
        isShow: true,//动态显示列
    ...
    methods:{
      ...
      //初始化导航名称
      initName: function () {
        let num = this.params.id.camponType;
        if(num == 1003) {
          this.name = '上架预占单据';
          this.camponName = '待上架数量';
          this.isShow = false
        }else{
          this.name = '可配货预占单据'
        }
