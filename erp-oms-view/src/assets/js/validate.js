...
//手机号中间显示****
//三位分节法
export function sectionThreePoints(num){
  return num.toLocaleString();
}
export function phoneEncrypt(value){
 var a = value.slice(0,3),b = value.slice(-4);
 return a + '****' + b;
}
//套装拆分剩余重组套装
...
export function handleData(arr){
  var mapArr = new Map(),newArr = [];
  arr.forEach(item=>{
    if(!mapArr.get(item.giftGoodsId)){
      item.totalFh = item.giveTotalQty;
  ...
  newArr.forEach(item=>{
    if(mapArr.get(item.giftGoodsId)){
      var newItem = mapArr.get(item.giftGoodsId);
      newItem.totalFh += item.giveTotalQty;
      mapArr.set(item.giftGoodsId,newItem);
    }
  })
  return mapArr;
}
...
export function gridOptions(){
  return {
    ...
    context: this,
    defaultColDef: {
      headerCheckboxSelectionFilteredOnly: true,//复选框将在选中时仅选择已筛选的行，并在取消选中时仅取消选择已筛选的行。该复选框将仅根据筛选的行更新其状态
...
//时间戳转成年月日 时分秒getMilliseconds
export function turnTimestamp1(time) {
  var a = new Date(new Date(time));
  return a.getFullYear() + '-' + turnSingle(a.getMonth() + 1) + '-' + turnSingle(a.getDate()) + ' ' + turnSingle(a.getHours()) + ':' + turnSingle(a.getMinutes()) + ':' + turnSingle(a.getSeconds()) +'.'+ a.getMilliseconds();
}
...
// 补零的方法（两个零）
export function returnFloat1(x) {
  if(isNaN(x) || x+''=='' || x==undefined || x<0){
    return '';
  }
  x += '';
