  ...
  <div id="invoicesControlSettingTax">
        ...
        <el-row>
          <el-col :span="6">
            <div class="grid-content">
              ...
              <el-input id="conPlanName2" v-model="listQuery.taxrateSetupName" clearable></el-input>
            </div>
          </el-col>
        </el-row>
      </div>
      ...
      <el-table id="tab_setting" :data="list" v-loading.body="listLoading"
        ...
        <el-table-column :label="$t('receipt.planName')" show-overflow-tooltip head-align="center">
          <template slot-scope="scope">
            <span class="link-type" @click="handleView(scope.row.taxrateSetupId)">{{scope.row.taxrateSetupName}}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="税率" show-overflow-tooltip align="center">
          <template slot-scope="scope">
            <span>{{taxrateTypeidObj[scope.row.rate]}}</span>
          </template>
        </el-table-column>
        <el-table-column :label="$t('receipt.operate')" align="center" width=130>
          <template slot-scope="scope">
            <el-button @click='handleEdit(scope.row.taxrateSetupId)'  type="text" :disabled="scope.row.enableFlag">
              {{$t("receipt.btnEdit")}}
            </el-button>
    ...
    <el-dialog v-dialogDrag :close-on-click-modal="false" :title="$t('receipt.tipTaxNumPlan')" :visible.sync="dialogEdit" v-loading="loading" element-loading-text="拼命加载中"
      ...
      <el-form ref="dataForm" :model="dataForm" :rules="rules" label-width="100px">
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('receipt.labPlanName')" prop="taxrateSetupName">
              <el-input v-model="dataForm.taxrateSetupName" :maxlength="40"></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="$t('receipt.labTaxNum')" prop="rate">
              <el-select v-model="dataForm.rate" @change="onTypeidChange" placeholder="请选择" style="width: 100%">
                <el-option v-for="item in condition.taxrateTypeidOptions" :key="item.value" :value="item.value" :label="item.label"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item :label="$t('receipt.labPlanName')">
              <el-input v-model="dataForm.taxrateSetupName" disabled></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="12">
            <el-form-item :label="$t('receipt.labTaxNum')">
              <el-select v-model="dataForm.rate" disabled placeholder="请选择" style="width: 100%">
                <el-option v-for="item in condition.taxrateTypeidOptions" :key="item.value" :value="item.value" :label="item.label"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
    ...
    <!--添加【商品、品牌、分类、店铺】过滤-->
    <el-dialog v-dialogDrag :close-on-click-modal="false" title="添加" :visible.sync="dialogSearch" width="780px" v-loading="loading" element-loading-text="拼命加载中"
               element-loading-spinner="el-icon-loading">
      ...
      <el-table :data="searchList" @selection-change="onSearchSel"
        ...
        <el-table-column v-if="dataForm.setupType === '1'" :label="$t('receipt.skuUrl')" align="center">
          <template slot-scope="scope">
            <!--<div v-if="goodsUrl != ''" style="position: fixed;z-index: 1000000000;left:50%;top:50%;border-radius: 10px;margin-top: -150px;margin-left: -150px;background: #000">
              <img :src="goodsUrl" alt="" style="width: 300px;height: 300px;cursor: pointer;border-radius: 10px;">
            </div>
            <img v-if="scope.row.goodsUrl != ''" :src="scope.row.goodsUrl" alt="" style="width: 25px;height: 25px;cursor: pointer" @mouseenter="showImg(scope.row,scope.$index)" @mouseleave="showImg1()">
            -->
            <el-tooltip placement="right">
              <div slot="content">
                <img v-if="scope.row.goodsUrl != ''" :src="scope.row.goodsUrl" alt="" style="max-width:300px;">
  ...
  export default {
    ...
    data() {
      return {
        ...
        rules: {
          taxrateSetupName: [{ required: true, message: this.$t('receipt.rulePlanName'), trigger: 'blur' }],
          rate: [{ required: true, message: this.$t('receipt.ruleTaxNum'), trigger: 'blur' }],
    ...
    methods: {
      ...
      getList() {
        ...
        this._post('com.edb01.erp.oms.service.api.OrderInvoiceService/' + this.apiVersion + '/getTaxRateList',this.listQuery).then(response => {
      ...
      handleView(taxrateSetupId) {
        ...
        this.beanList = [];
        this._post('com.edb01.erp.oms.service.api.OrderInvoiceService/' + this.apiVersion + '/getSingleTaxRate',{taxrateSetupId}).then(response => {
          if(response.data.success){
      ...
      handleDelete(row) {
        this.$confirm(this.$t('receipt.detelePlan'), this.$t('receipt.handlePrompt'), {
          ...
          const item = { taxrateSetupId: row.taxrateSetupId }
          //deleteReceiptTaxPlan(item)
          this._post('com.edb01.erp.oms.service.api.OrderInvoiceService/' + this.apiVersion + '/deleteTaxrateSetup',item).then(response => {
            if (response.data.success) {
              this.$message({
                message: '删除成功',
                type: 'success'
      ...
      handleEnable(row) {
        this.$confirm('是否'+ (row.enableFlag ? '启用':'停用') +'该方案?', this.$t('orderSign.prompt'), {
          ...
          const item = { taxrateSetupId: row.taxrateSetupId, isEnable: row.enableFlag }
          //enableReceiptTaxPlan(item)
          this._post('com.edb01.erp.oms.service.api.OrderInvoiceService/' + this.apiVersion + '/isEnableTaxrateSetup',item).then(response => {
            if (response.data.success) {
      ...
      //添加或编辑
      handleEdit(taxrateSetupId) {
        if (this.list.length >= 20 && taxrateSetupId == '') {
          this.$message({
            message: '最多可添加20条方案！',
            type: 'warning'
          });
        ...
        this.initDataForm(taxrateSetupId)
      ...
      onSearch() {
        this.loading = true;
        this._post('com.edb01.erp.oms.service.api.OrderInvoiceService/' + this.apiVersion + '/addOrderInvoiceTaxRateSetType',this.paramData).then(response => {
          this.loading = false;
          if(response.data.success){
            this.dialogSearch = true;
            /*var searchList = [];
            if(this.dataForm.setupType == 3){
              searchList = response.data.result.result.rows;
              this.totalNum = response.data.result.result.total;
            }else{
              searchList = response.data.result.rows;
      ...
      confirmEdit() {
        ...
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this._post('com.edb01.erp.oms.service.api.OrderInvoiceService/' + this.apiVersion + '/addTaxrateSetup',this.dataForm).then(response => {
              if (response.data.success) {
                this.$message({
                  message: '保存成功',
                  type: 'success'
