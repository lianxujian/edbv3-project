  ...
  export default {
    ...
    watch:{
      pulicLogVisible:function (a,b) {
        if(!b){
          this.paramData.sourceType=this.paramObj.moduleId?this.paramObj.moduleId:0;//模块Id
          this.paramData.sourceId=this.paramObj.billCode?this.paramObj.billCode:'';//数据主键Id（单条数据Id）
          this.paramData.operateTypes=this.paramObj.typeId?this.paramObj.typeId:'';//操作类型（添加，修改，删除）
          this.renderFn();
        }
      }
    },
    mounted() {
      this.renderFn=()=>{
        ...
        this.loading = true;
        this._post(process.env.BASE_API + 'com.edb01.erp.mdata.service.api.OperateLogService/'+this.version+'/getLogDataPager',paramData).then((response)=>{
          this.loading = false;
