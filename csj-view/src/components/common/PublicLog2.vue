  ...
  export default {
    ...
    methods: {
      queryBtn(){
        ...
        this._post(process.env.BASE_API + 'com.edb01.erp.mdata.service.api.OperateLogService/'+this.version+'/getLogList',paramData).then((response)=>{
          if(response.data.code==0){
            this.publicLogData = response.data.result;
            this.publicLogData.forEach((item, i) => {
              if(item.operateTime)item.operateTime = turnTimestamp(item.operateTime);
            })
          }else{
            this.$message({
