  ...
  export default {
    ...
    methods: {
      ...
      saveAmendData() {
        ...
        self._post('com.edb01.risk.service.api.iface.WhiteIpApi/' + self.version + '/update',self.amendD)
          .then(function(res) {
            if (res.data.code === 0) {
      ...
      addD() {
        ...
        self._post('com.edb01.risk.service.api.iface.WhiteIpApi/' + self.version + '/insert',self.addData)
          .then(function (res) {
            if (res.data.code === 0) {
      ...
      DelClick(row) {
        ...
        self.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
         ...
         /* self.$axios({
          ...
          self._get('com.edb01.risk.service.api.iface.WhiteIpApi/' + self.version + '/deleteById?id=' + row.id + '&clToken=' + self.token)
            .then(function(res) {
              if (res.data.code === 0) {
                self.queryData(self.parameter);
                self.$message({
    ...
    /*  if (localStorage.getItem('clToken')) {
      ...
      this.queryData = (par) => {
        var self = this;
        self._post('com.edb01.risk.service.api.iface.WhiteIpApi/' + self.version + '/getByPage',par)
          .then(function(res) {
            if (res.data.code === 0) {
