  ...
  export default {
    ...
    methods: {
      ...
      save() {
        ...
        if (!self.dis) {
          ...
          self._post('com.edb01.risk.service.api.iface.RiskRulesApi/' + self.version + '/saveUpdate',par)
            .then(function(res) {
              if (res.data.code === 0) {
                self.dis = true;
                self.$message({
    ...
    created() {
     /* if (localStorage.getItem('clToken')) {
      ...
      this.queryD = () => {
        var self = this;
        self._get('com.edb01.risk.service.api.iface.RiskRulesApi/' + self.version + '/getByType?type=' + self.logo + '&clToken=' + self.token)
          .then(function(res) {
            if (res.data.code === 0) {
