...
<script>
  export default {
    ...
    methods: {
      ...
      handleClick(row) {
        ...
        self.$confirm('是否解封?', '提示', {
          ...
          self.get('com.edb01.risk.service.api.iface.BlackIpApi/' + self.version + '/unbanByIp?ip=' + row.ip + '&clToken=' + self.token)
            .then((res) => {
              if (res.data.code == 0) {
    ...
    /*  if (localStorage.getItem('clToken')) {
      ...
      this.queryD = (par) => {
        var self = this;
        self._post('com.edb01.risk.service.api.iface.BlackIpApi/' + self.version + '/getByPage',par)
          .then(function(res) {
            if (res.data.code === 0) {
